...
void Loss::findKBest(
  ...
  for (int32_t i = 0; i < output.size(); i++) {
    if (output[i] < threshold) {    //Greater than a threshold condition
      continue;
    }
    // Finding Top k with least loss
...
//This Function to be used as it is in MOE
real BinaryLogisticLoss::binaryLogistic(    //Returns binary Logistic Loss
    int32_t target,
    Model::State& state,
  ...
  Vector& hidden = isMisspelled ? state.missHidden : state.hidden;
  real score = sigmoid(wo_->dotRow(hidden, target));
//  if(isMisspelled)
//    score = sigmoid(wo_->dotRow(state.missHidden, target));
//  else
//    score = sigmoid(wo_->dotRow(state.hidden, target));
...
//    if(isMisspelled)
//      wo_->addVectorToRow(state.missHidden, target, alpha);
//    else
//      wo_->addVectorToRow(state.hidden, target, alpha);
  }
  if (labelIsPositive) {    //Positive Sample
    return -log(score);
...
void BinaryLogisticLoss::computeOutput(Model::State& state) const {
  ...
  //Multiply matrix row by row with the hidden vector
  // and store output in output vector
...
real OneVsAllLoss::forward(
    ...
    bool backprop,
    bool isMisspelled,
    const std::vector<int32_t>* missinput) {
  real loss = 0.0;
...
real NegativeSamplingLoss::forward(
    ...
    int32_t targetIndex,                    //Chosen word within the range c
    Model::State& state,
    real lr,
    bool backprop,
    bool isMisspelled,
  ...
  int32_t target = targets[targetIndex];    // Chosen word in the c range
  real loss = isMisspelled ? binaryLogistic(target, state, true, lr, backprop, true) : loss = binaryLogistic(target, state, true, lr, backprop);
//  if(isMisspelled)
//    loss = binaryLogistic(target, state, true, lr, backprop, true);
//  else
//    loss = binaryLogistic(target, state, true, lr, backprop);


  for (int32_t n = 0; n < neg_; n++) {
    auto negativeTarget = getNegative(target, state.rng);
    loss = isMisspelled ? binaryLogistic(negativeTarget, state, false, lr, backprop, true): binaryLogistic(negativeTarget, state, false, lr, backprop);
//    //Cleared a bug here
//    if(isMisspelled)
//      loss = binaryLogistic(negativeTarget, state, false, lr, backprop, true);
//    else
...
//Implementing the MOEloss class constructor
MOEloss :: MOEloss(
  std::shared_ptr<Matrix>& wo,
  int neg,
  const std::vector<int64_t>& targetCounts,
...
//Implementing MOEloss Class Functions
real MOEloss :: forward(
  const std::vector<int32_t> &targets,      //line
  int32_t targetIndex,                      //chosen word
  fasttext::Model::State &state,
  fasttext::real lr,
  bool backprop,
  bool isMisspelled,
  const std::vector<int32_t>* missTargets) {  //Misspelled Line

  real Lft = NegativeSamplingLoss :: forward(targets, targetIndex, state, lr, backprop);
  real Lsc = NegativeSamplingLoss :: forward(*missTargets, 1, state, lr, backprop, true);
  // alpha to be extracted from args/state
  //Need to see how to extract the sizes of the datasets
   real Lmoe = (1 - moeAlpha_) * Lft + ( moeAlpha_ * (abs(corrSize_) / abs(missSize_)) ) * Lsc;
   return Lmoe;
}
