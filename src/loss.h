...
class Loss {
 ...
 public:
  ...
  virtual real forward(
    ...
    const std::vector<int32_t>* missinput = nullptr) = 0;
...
class OneVsAllLoss : public BinaryLogisticLoss {
 public:
  ...
  real forward(
      ...
      bool backprop,
      bool isMisspelled = false,
      const std::vector<int32_t>* missinput = nullptr) override;
};
...
//New MOEloss class is being implemented
class MOEloss : public NegativeSamplingLoss {
protected:
    int32_t missSize_;    //misspelled_size_
    int32_t corrSize_;   //nwords_ - misspelled_size_
    real moeAlpha_;
...
public:
    explicit MOEloss(
      ...
      int32_t corrSize,
      real moeAlpha);
    ~MOEloss() noexcept override  = default;
    real forward(
      const std::vector<int32_t >& targets,
      int32_t targetIndex,
      Model::State& state,
