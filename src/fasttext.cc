...
std::shared_ptr<Loss> FastText::createLoss(std::shared_ptr<Matrix>& output) {
  ...
  switch (lossName) {
    ...
    // Creating Loss for MOE
    case loss_name :: moeLoss:
      return std::make_shared<MOEloss>(
         output, args_->neg, getTargetCounts(),
         dict_->misspelled_words(),
         dict_->nwords() - dict_->misspelled_words(),
         args_ -> moeAlpha);
...
void FastText::skipgram(
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    //Creating skipgram pairs around c distance in the words
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        //updating the loss of the model according to the pairs created
...
// Defining moeFunc function for the forwarding of the MOEloss
void FastText :: moeFunc(
      Model :: State& state,
      real lr,
      const std::vector<int32_t>& line,
      const std::vector<int32_t>& misspelledLine){
  std :: uniform_int_distribution<> uniform(1, args_->ws);
  for (int32_t w = 0; w < line.size(); w++){
    int32_t boundary = uniform(state.rng);
    ...
    //getting ngram of the misspelled word in the misspelledLine
    const std::vector<int32_t>& misspelledNgrams = dict_->getSubwords(misspelledLine[0]);

    for(int32_t c = -boundary; c <= boundary; c++){
      if(c != 0 && w + c >=0 && w + c < line.size()) {
        // Misspelled as parameter to be passed here
        model_->update(ngrams, line, w+c, lr, state, &misspelledNgrams, &misspelledLine);
      }
    }
...
void FastText::test(std::istream& in, int32_t k, real threshold, Meter& meter)
  ...
  //Call Corrected after adding the missHidden size
  Model::State state(args_->dim, args_->dim, dict_->nlabels(), 0);
  in.clear();
...
void FastText::predict(
  ...
  //Call corrected after adding the missHiddenSize
  Model::State state(args_->dim, args_->dim, dict_->nlabels(), 0);
  if (args_->model != model_name::sup) {
...
void FastText::precomputeWordVectors(DenseMatrix& wordVectors) {
  Vector vec(args_->dim);
  //Word Vectors of all the words in the dict of size nwords_
  wordVectors.zero();
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    std::string word = dict_->getWord(i);
    //Vector of each word is the summation of the ngrams of that word
    getWordVector(vec, word);
    real norm = vec.norm();
    if (norm > 0) {
...
std::vector<std::pair<real, std::string>> FastText::getNN(
  ...
  // Word Vectors in fasttext are the summation of the embeddings of the ngrams
  getWordVector(query, word);

  //returns the word vectors of all the words as the sumamtion of their ngrams
...
void FastText::trainThread(int32_t threadId) {
  ...
  //Taking the misspelled dataset input
  std::ifstream ims;
  if(!(args_->misspelledData.empty())){
    ims.open(args_->misspelledData);
    utils::seek(ims, threadId * utils::size(ims) / args_->thread);
  }
  //Call Corrected after adding the missHiddenSize
  Model::State state(args_->dim, args_->dim, output_->size(0), threadId + args_->seed);

  const int64_t ntokens = dict_->ntokens();

  // Misspelled Changes
  int64_t localTokenCount = 0;
  int64_t misspelledTokenCount = 0;
  std::vector<int32_t> line, labels, misspelledLine;
  try {
    while (keepTraining(ntokens)) {
      ...
      // Adding the moe condition
      // Look into how are the line is selected till the end
      else if (args_->model == model_name :: moe) {
        localTokenCount += dict_->getLine(ifs, line, state.rng);
        misspelledTokenCount += dict_->getLine(ims, misspelledLine, state.rng);
        moeFunc(state, lr, line, misspelledLine);
      }

      //Deliberately not added the misspelledTokenCount here
      // See if it makes any defference
...
std::shared_ptr<Matrix> FastText::getInputMatrixFromFile(
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    //modified according to misspelled iscorrect
    dict_->add(word);//, true);
  ...
  std::shared_ptr<DenseMatrix> input = std::make_shared<DenseMatrix>(
      //missBucket added once again and removed it later
...
std::shared_ptr<Matrix> FastText::createRandomMatrix() const {
  ...
  // n x m === (words + ngrams(bucket size) + misspelled bucket size ) x dimension of embedding
  //May be required to be changed
  //Remodification is done here by removing the missBucket
  std::shared_ptr<DenseMatrix> input = std::make_shared<DenseMatrix>(
      dict_->nwords() + args_->bucket, args_->dim);
  input->uniform(1.0 / args_->dim, args_->thread, args_->seed);
...
void FastText::train(const Args& args) {
  ...
  std::ifstream ims(args_->misspelledData);
  if (!ims.is_open()) {
    throw std::invalid_argument(
      args_->misspelledData + " cannot be opened for training!");
  }

  dict_->readFromFile(ifs, ims);
  ifs.close();
  ims.close();
  ...
  if (!args_->pretrainedVectors.empty()) {
    ...
    // This is the word embedding matrix
    input_ = createRandomMatrix();      //This is wi_ in the model
  }
  // This is the wo_ in the model object which is initialized to zero
  // This is the output transformation matrix
  output_ = createTrainOutputMatrix();
  quant_ = false;

  //moe functionality added in createLoss function
  auto loss = createLoss(output_);      //Creating Loss
  bool normalizeGradient = (args_->model == model_name::sup);
  model_ = std::make_shared<Model>(input_, output_, loss, normalizeGradient);
