...
Args::Args() {
  ...
  wordNgrams = 1;
  //loss = loss_name::ns;
  loss = loss_name::moeLoss;
  //model = model_name::sg;
  model = model_name::moe;

  bucket = 2000000;

//  //Defining bucket size for misspelled words
  missBucket = 2000000;

  //Defining moeAlpha hyperparameter
  moeAlpha = 0.3;
...
void Args::parseArgs(const std::vector<std::string>& args) {
  ...
  for (int ai = 2; ai < args.size(); ai += 2) {
    ...
    try {
      ...
      //Added Misspelled Data Input argument function case
      else if (args[ai] == "-misspelledData") {
          misspelledData = std::string(args.at(ai+1));
      }
...
//      for inputting the missBucket size
      else if (args[ai] == "-missBucket"){
        missBucket = std::stoi(args.at(ai + 1));
      }

      //for inputting the moeAlpha
      else if (args[ai] == "-moeAlpha"){
        moeAlpha = std::stoi(args.at(ai + 1));
      }
  ...
  //Added Misspelled Data Condition
  if (input.empty() || output.empty() || misspelledData.empty()) {
...
void Args::printDictionaryHelp() {
  std::cerr << "\nThe following arguments for the dictionary are optional:\n"
            ...
            << "  -missBucket         number of buckets for misspelled Words [" << missBucket << "]\n"
            << "  -moeAlpha           Alpha used in Lmoe Loss Function" << moeAlpha << "]\n"
...
void Args::printTrainingHelp() {
  std::cerr
      ...
      << "  -loss               loss function {ns, hs, softmax, one-vs-all, moe} ["
      << lossToString(loss) << "]\n"
...
void Args::save(std::ostream& out) {
  ...
  out.write((char*)&(missBucket), sizeof(int));
...
void Args::load(std::istream& in) {
  ...
  in.read((char*)&(missBucket), sizeof(int));
