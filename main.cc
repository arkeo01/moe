...
void nn(const std::vector<std::string> args) {
  ...
  //fetching the query word for similarity
...
////Defining function for testing Args
//void testArgs(const std::vector<std::string>& args){
//  Args a = Args();
//  a.parseArgs(args);
//  std::shared_ptr<Args> args_ = std::make_shared<Args>(a);
...
int main(int argc, char** argv) {
  ...
  if (command == "skipgram" || command == "cbow" || command == "supervised" || command == "moeModel") {
    train(args);
  } else if (command == "test" || command == "test-label") {
    test(args);
  }
  //Testing function for args
//  else if (command == "testArgs") {
//      testArgs(args);
//  }
